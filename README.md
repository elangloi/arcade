...
Players only ever see the shell (`/`, `/single/...`, `/multiplayer/...`); it frames the games, which the proxy serves by path (`/teen-titans-battle-blitz/`, `/lilo-and-stitch-sandwich-stacker/`, `/club-penguin-pizzatron/`, `/noby-noby-boy/`, and the multiplayer service at `/arcade/multiplayer/`). See `compose/README.md` for adding more.
