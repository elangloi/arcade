  ...
  if abs(diff.locH) < me.RANGE_THRESHOLD_X_SHORT then
    if me.targetAtSceneEdge then
      if choice < 35 then
        ...
        me.target.queueMove(g.MOVE_MAMMOTH_JUMP_PUNCH, 0)
      end if
    else
        ...
        if (me.opponentCurrVel.locH * me.opponentCurrDir) >= 0.0 then
          ...
          if choice < 30 then
            ...
            if choice < 75 then
              ...
              me.target.queueMove(g.MOVE_MAMMOTH_SHOULDER_BARGE, 0)
            end if
          end if
        end if
      else
      ...
      if me.opponentCurrMoveID = g.MOVE_JUMP then
        if (me.opponentCurrVel.locH * me.opponentCurrDir) >= 0.0 then
          ...
          if choice < 50 then
            ...
            me.target.queueMove(g.MOVE_MAMMOTH_GROUND_PUNCH, 0)
          end if
        end if
      else
        if choice < 50 then
...
on evadeRangedAttack me, attPos, attVel, framesLeft
  choice = random(100) - 1
  x = me.targetCurrPos.locH
  y = me.targetCurrPos.locV
  diffX = attPos.locH - x
  diffY = attPos.locV - y
