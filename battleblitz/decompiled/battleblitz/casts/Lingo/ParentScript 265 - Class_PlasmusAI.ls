...
on new me, obj
  ...
  me.RANGE_THRESHOLD_X_SHORT = 180
  me.RANGE_THRESHOLD_X_MEDIUM = 400
...
on selectAttackMove me
  ...
  if abs(diff.locH) < me.RANGE_THRESHOLD_X_SHORT then
    if me.targetAtSceneEdge then
      if choice < 100 then
        me.target.queueMove(g.MOVE_PLASMUS_ARM_SWIPE, 0)
      end if
    else
      if me.opponentCurrMoveID = g.MOVE_JUMP then
        if (me.opponentCurrVel.locH * me.opponentCurrDir) >= 0.0 then
          ...
          if choice < 25 then
            ...
            if choice < 50 then
              me.target.queueMove(g.MOVE_PLASMUS_SLUDGE_THROW, 0)
            else
              if choice < 75 then
                me.target.queueMove(g.MOVE_PLASMUS_CRAB_THROW, 0)
              else
                me.target.queueMove(g.MOVE_PLASMUS_CHEST_BLAST, 0)
              end if
            end if
          end if
        end if
