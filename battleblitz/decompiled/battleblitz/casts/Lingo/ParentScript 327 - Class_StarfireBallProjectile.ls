...
on new me, owner, initPos, initVel, initDir
  ...
  me.attackDamage = g.DAMAGE_STARFIRE_ENERGY_BALL
  me.stunDuration = 20
  initState = 1
  vis = [g.assets.STARFIRE.FX_STARFIRE_BALL_01]
  att = [g.assets.STARFIRE.FX_STARFIRE_BALL_01A]
...
on update me
  ...
  if me.alive then
    if initState then
      initState = 0
      g.main.audioMgr.playSound(g.assets.STARFIRE.SFX_STARFIRE_STARBOLT_THROW, 100, g.SFX_EVENT_PRIORITY_LOW)
    end if
    if me.age > 1 then
...
on triggerPayoff me
  ...
  if me.owner.opponent.isBlocking() or not me.owner.opponent.getMove().isVulnerable() then
    g.main.audioMgr.playSound(g.assets.STARFIRE.SFX_STARFIRE_STARBOLT_MISS, 100, g.SFX_EVENT_PRIORITY_LOW)
    ...
    g.main.audioMgr.playSound(g.assets.STARFIRE.SFX_STARFIRE_STARBOLT_HIT, 100, g.SFX_EVENT_PRIORITY_LOW)
    return 1
  end if
end
