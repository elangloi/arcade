    ...
    if me.targetAtSceneEdge then
      if choice < 25 then
        ...
        if choice < 50 then
          ...
          me.target.queueMove(g.MOVE_RAVEN_TELEKINETIC_SPIN, 0)
        end if
      end if
    else
          ...
          if choice < 25 then
            ...
            if choice < 50 then
              ...
              if choice < 75 then
                ...
                me.target.queueMove(g.MOVE_RAVEN_EXPLOSIVE_DROP, 0)
              end if
            end if
          end if
        end if
    ...
    if diff.locH < me.RANGE_THRESHOLD_X_MEDIUM then
      if me.opponentCurrMoveID = g.MOVE_JUMP then
        if (me.opponentCurrVel.locH * me.opponentCurrDir) >= 0.0 then
          ...
          if choice < 50 then
            me.target.queueMove(g.MOVE_RAVEN_TELEKINETIC_THROW, 0)
          else
            me.target.queueMove(g.MOVE_RAVEN_EXPLOSIVE_DROP, 0)
          end if
        end if
