...
uv run python compose.py --game ../../<game> <spriteId|main> <frame> out.svg [--pad 4]
...
movie = json.load(open(ASSETS / 'movie.json'))
defs_doc = etree.parse(str(ASSETS / 'defs.svg'))
...
def compose(parent, sid, frame, used):
    ...
    for d, rec in sorted(lst.items(), key=lambda kv: int(kv[0])):
        if rec.get('clip'):
            continue   # mask layers aren't drawn (and their content is left unmasked here)
        cid = str(rec['id'])
