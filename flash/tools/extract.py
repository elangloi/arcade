...
uv run python extract.py --game ../../<game>
...
from common import ASSETS as OUT
...
def timeline(tags, swf_version):
    ...
    for code, name, body in tags:
        if name == 'ShowFrame':
        ...
        elif name in ('PlaceObject2', 'PlaceObject3'):
            po = swfraw.place_object2(body, swf_version, v3=(name == 'PlaceObject3'))
            d = po['depth']
            ...
            if 'filters' in po: obj['filters'] = po['filters']
            if 'visible' in po: obj['visible'] = po['visible']
...
def main():
    ...
    for code, name, tb in tags:
        ...
        elif name == 'ExportAssets':   # linkage names for attachMovie / attachSound
            r = Reader(tb); n = r.u16()
            for _ in range(n):
                cid = r.u16(); movie['exports'][r.string()] = cid
        elif name == 'DefineFont3':
            font3.add(struct.unpack('<H', tb[:2])[0])
        elif name == 'DoAction':
            # Club Penguin locale files: `var localeText = [{value: '...', id: '...'}, ...]`
            import avm1, re, ast
            txt = avm1.decompile(tb)
            for v, k in re.findall(r"\{value: ('(?:[^'\\]|\\.)*'), id: ('(?:[^'\\]|\\.)*')\}", txt):
                movie['locale'][ast.literal_eval(k)] = ast.literal_eval(v)
    ...
    # DefineFont2 glyphs are in twips (-> 20/1024); DefineFont3 glyphs are 20x finer, so 1/1024 is right.
    for path in keep.iter('{http://www.w3.org/2000/svg}path'):
        pid = path.get('id') or ''
        if pid.startswith('font_') and path.get('transform') == 'scale(0.0009765625)':
            fid = int(pid.split('_')[1])
            if fid not in font3:
    ...
    for t in s.all_tags_of_type(TagDefineFont2):   # (TagDefineFont3 subclasses it)
        em = 20480 if t.characterId in font3 else 1024   # Font3 metrics are 20x finer
        movie['fonts'][str(t.characterId)] = {
            'name': (t.fontName.decode('latin-1') if isinstance(t.fontName, bytes) else str(t.fontName)).rstrip('\0'),
            'ascent': getattr(t, 'ascent', 0) / em, 'descent': getattr(t, 'descent', 0) / em,
            'advances': {str(c): a / em for c, a in zip(t.codeTable, t.fontAdvanceTable)} if getattr(t, 'fontAdvanceTable', None) else {},
